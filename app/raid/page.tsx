...
type RaidEvent = { kind:string; seq:number; attackerId?:string; targetId?:string; targetSlot?:number; damage?:number; name?:string; defeatedName?:string; side?:string; defeated?:boolean };
...
export default function RaidPage() {
  ...
  return (
    <main className="raid-page raid-combat-page"><header className="raid-header"><div><p className="eyebrow">QUINTESSON BOSS RUSH · ROUND {state.round}</p><h1>{state.stage==="victory" ? "Boss Rush Victory" : state.stage==="defeat" ? "Boss Rush Defeat" : state.stage==="boss" ? "Boss Turn" : state.stage==="reposition" ? "Repositioning" : active ? "Your Turn" : `${activeName || "Your ally"}'s Turn`}</h1></div><div className="raid-turn-control">{active && state.stage === "combat" ? <button className="primary raid-end" onClick={() => socket?.emit("raid-end-turn")}>End Your Turn</button> : null}{state.stage === "reposition" && moving ? <button className="primary raid-end" onClick={() => { setMoveSource(null); setMessage("Reposition skipped. Waiting for your ally."); socket?.emit("raid-skip-reposition"); }}>Skip reposition</button> : null}</div><b>{active ? `${state.actions} attacks` : state.stage==="reposition" ? `${state.repositions[state.youId] || 0} move` : "Stand by"}</b>{finished ? <Link className="ghost" href="/">Return to menu</Link> : null}</header>
      ...
      {deathNotice ? <div className="raid-death-overlay" role="dialog" aria-modal="true" aria-labelledby="raid-death-title"><section className="raid-death-popup"><button className="raid-death-close" aria-label="Close Tribunal death notice" onClick={closeDeathNotice}>×</button><CardImage src={state.judge.image} alt="Quintesson Judge"/><div><p className="raid-death-kicker">TRIBUNAL DISPOSAL</p><h2 id="raid-death-title">{deathNotice.name} was thrown to the sharkticons</h2><p>The Judge records the loss. Choose your next move carefully.</p></div></section></div> : null}
    </main>
  );
}
