...
export function attackLimit({ unit, board, deck, grimlockFrenzyRound, round }) {
  ...
  if (abilityKey(unit) === "grimlock" && grimlockFrenzyRound === round) return 3;
  if (deployedAttackers < 3) return 3;
  if (unit.id === "airrazor" && deck.filter((card) => card.faction === "Maximal").length >= 3) return 2;
  if (unit.id === "sun" && board.some((card) => card?.id === "side") && unit.abilityUses > 0) return 2;
  return 1;
}
...
export function applyCharacterAttackDamage(unit, amount, reduction = 0) {
  ...
  if (hitStreak >= 2 && unit.abilityUses > 0) {
    return {
      unit: { ...result.unit, hp: unit.max, abilityUses: unit.abilityUses - 1, dinobotHitStreak: 0 },
      damage: result.damage,
      blocked: result.blocked,
      restored: true,
    };
  }
  return { ...result, unit: { ...result.unit, dinobotHitStreak: hitStreak }, restored: false };
}
...
export function applyBoardAuras(slots) {
  ...
  const maximalGrimlockActive = slots.some((unit) => unit?.id === "maxgrimlock");
  const optimalSupport = slots.some((unit) => unit?.id === "primal" || unit?.id === "optimus");
  return slots.map((unit) => {
    if (!unit) return null;
    let next = unit;
    const diveShouldApply = divebombActive && unit.faction === "Predacon" && unit.id !== "dive";
    if (diveShouldApply && !next.diveBonus) next = { ...next, max: next.max + 10, hp: next.hp + 10, diveBonus: true };
    else if (!diveShouldApply && next.diveBonus) next = { ...next, max: next.max - 10, hp: Math.max(1, next.hp - 10), diveBonus: false };
    const frenzyShouldApply = rumbleActive && next.id === "frenzy";
    if (frenzyShouldApply && !next.frenzyBonus) next = { ...next, max: next.max + 20, hp: next.hp + 20, frenzyBonus: true };
    else if (!frenzyShouldApply && next.frenzyBonus) next = { ...next, max: next.max - 20, hp: Math.max(1, next.hp - 20), frenzyBonus: false };
    const dinobotShouldGain = maximalGrimlockActive && next.id === "dinobot";
    if (dinobotShouldGain && !next.dinobotBonus) next = { ...next, abilityUses: next.abilityUses + 1, dinobotBonus: true };
    else if (!dinobotShouldGain && next.dinobotBonus) next = { ...next, abilityUses: Math.min(next.abilityUses, 1), dinobotBonus: false };
    if (optimalSupport && abilityKey(next) === "optimal" && !next.optimalBoost) next = { ...next, dmg: 30, optimalBoost: true };
    return next;
  });
}
...
export function canRhinoxRevive(rhinox, defeated) {
  return abilityKey(rhinox) === "rhinox" && rhinox.hp > rhinox.max / 2 && rhinox.abilityUses > 0 && defeated.some((unit) => unit.faction === "Maximal");
}

export function reviveAtHalf(unit) {
  return { ...unit, hp: Math.ceil(unit.max / 2), canAttack: true, poison: 0, dinobotHitStreak: 0 };
}

export function shouldLayDepthchargeMine(board, defeated) {
  return defeated?.faction === "Maximal" && board.some((unit) => abilityKey(unit) === "depthcharge");
}
