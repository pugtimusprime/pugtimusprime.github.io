...
function validatePng(assetPath) {
  ...
  while (offset + 12 <= data.length) {
    ...
    assert.ok(
      ...
      `${assetPath} contains a complete ${type} chunk`,
    );
    if (type === "IDAT")
  ...
  assert.doesNotThrow(
    ...
    `${assetPath} image data decompresses`,
  );
}
...
test("the repaired large cards also exist in the GitHub Pages root", () => {
  assert.deepEqual(
    readAsset("/public/cards/characters/depthcharge.png"),
    readAsset("/cards/characters/depthcharge.png"),
  );
  assert.deepEqual(
    readAsset("/public/cards/battle/war-dawn.svg"),
    readAsset("/cards/battle/war-dawn.svg"),
  );
});

test("the six new Character Cards are mirrored into the GitHub Pages root", () => {
  for (const name of [
    "quickstrike",
    "tarantulas",
    "lio-convoy",
